...
pub fn narrowphase_main(
    ...
    if pair_index < pc.pair_count {
        ...
        // analytic sphere-sphere fast path: exact and cheap
        if support::both_spheres(&a.shape, &b.shape) {
            ...
            if distance >= radius || distance <= f32::EPSILON {
                ...
                let slot = &mut contacts.slots[pair_index as usize];
                slot.valid = 1;
                slot.a = pair.a;
                slot.b = pair.b;
                slot.depth = radius - distance;
                slot.normal = normal.extend(0.0);
                slot.point_a = (a.pos() + normal * a.shape.corners[0].x).extend(0.0);
                slot.point_b = (b.pos() - normal * b.shape.corners[0].x).extend(0.0);
            }
        } else {
            ...
            if gjk::gjk(a, b, &mut simplex) {
                match epa::epa(a, b, &simplex) {
                    Some(pen) => {
                        ...
                        slot.a = pair.a;
                        slot.b = pair.b;
                        slot.depth = pen.depth;
                        slot.normal = pen.normal.extend(0.0);
                        slot.point_a = pen.point_a.extend(0.0);
                        slot.point_b = pen.point_b.extend(0.0);
                    }
                    None => {}
                }
            }
